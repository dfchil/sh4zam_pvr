...
#include <enDjinn/enj_enDjinn.h> /* enDjinn main header */
...
#embed "../embeds/part_4_pvr_sprites/texture/rgb565_vq_tw/sh4zam256.dt"
};
static const alignas(32) uint8_t texture128_raw[] = {
#embed "../embeds/part_4_pvr_sprites/texture/argb1555_vq_tw/sh4zam128_t.dt"
};
static const alignas(32) uint8_t texture32_raw[] = {
#embed "../embeds/part_4_pvr_sprites/texture/pal4/sh4zam32_w.dt"
    // #embed "../build/pvrtex/rgb565_vq_tw/sh4zam32.dt"
};
static const alignas(32) uint8_t palette32_raw[] = {
#embed "../embeds/part_4_pvr_sprites/texture/pal4/sh4zam32_w.dt.pal"
};

static alignas(32) enj_texture_info_t texture256x256;
static alignas(32) enj_texture_info_t texture128x128;
static alignas(32) enj_texture_info_t texture32x32;
...
static inline void set_cube_transform(float scale) {
  ...
  shz_xmtrx_init_translation(cube_state.pos.x, cube_state.pos.y,
                             cube_state.pos.z);
  shz_xmtrx_apply_scale(scale * MODEL_SCALE * ENJ_XSCALE, scale * MODEL_SCALE,
                        scale * MODEL_SCALE);
...
void render_txr_tr_cube(void *__unused) {
...
void render_cubes_cube(void *__unused) {
  set_cube_transform(1.0f);
  ...
  if (render_mode == CUBES_CUBE_MAX) {
    cuberoot_cubes = 17 - (ENJ_SUPERSAMPLING * 1);
  ...
  int xiterations =
      cuberoot_cubes -
      (ENJ_SUPERSAMPLING == 0 && render_mode == CUBES_CUBE_MAX ? 1 : 0);
...
void render_wire_cube(void *__unused) {
  set_cube_transform(1.0f);
  alignas(32) shz_vec4_t tverts[8] = {0};
  for (int i = 0; i < 8; i++) {
    tverts[i] = shz_xmtrx_transform_vec4(cube_vertices[i]);
...
static inline void update_state(void *__unused) {
  ...
  switch (render_mode) {
  case TEXTURED_TR:
    enj_render_list_add(PVR_LIST_TR_POLY, render_txr_tr_cube, NULL);
    break;
  case WIREFRAME_FILLED:
  case WIREFRAME_EMPTY:
    enj_render_list_add(PVR_LIST_OP_POLY, render_wire_cube, NULL);
    break;
  case CUBES_CUBE_MAX:
    enj_render_list_add(PVR_LIST_OP_POLY, render_cubes_cube, NULL);
    break;
  case CUBES_CUBE_MIN:
    enj_render_list_add(PVR_LIST_PT_POLY, render_cubes_cube, NULL);
    break;
  default:
    break;
  }
...
int main(void) {
  ...
  if (!enj_texture_load_blob(&texture256_raw, &texture256x256))
    return -1;
  if (!enj_texture_load_blob(&texture128_raw, &texture128x128))
    return -1;
  // if (!pvrtex_load_palette_blob(palette64_raw, PVR_PAL_ARGB1555, 0))
  //   return -1;
  if (!enj_texture_load_blob(&texture32_raw, &texture32x32))
    return -1;
  if (!enj_texture_load_palette_blob(palette32_raw, PVR_PAL_RGB565, 256))
    return -1;

  enj_mode_t main_mode = {
      .name = "Main Mode",
      .mode_updater = update_state,
      .data = NULL,
  };
